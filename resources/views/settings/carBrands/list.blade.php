...
@section('dashboard')
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">List of Car Brand</h1>
                </div>
            </div>
        </div>
    </div>
                ...
                @endif
                    <table id="location" class="table table-bordered table-striped">
                        ...
                        @foreach($data as $datum)
                            <tr>
                                <td>{{++$i}}</td>
                                <td>{{$datum->cartype->name}}</td>
                                <td>{{$datum->name}}</td>
                                <td>{{$datum->status}}</td>
                                <td>
                                    <form action="{{ route('car-brands.destroy',$datum->id)}}" method="POST">
                                        @CSRF
                                        @method('DELETE')
                                        <a href="{{route('car-brands.edit',$datum->id)}}"
